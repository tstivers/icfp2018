...
namespace Contest.Core.Helpers
{
    public static class AstarGroundFinder
    {
        ...
        public static HashSet<Voxel> CanReachGround(Voxel start, Voxel avoid, Matrix map)
        {
            ...
            while (open_set.Count > 0)
            {
                ...
                if (current.Y == 0)
                {
                    ...
                    return ReconstructPath(came_from, start, current);
                }
        ...
        private static HashSet<Voxel> ReconstructPath(Dictionary<Voxel, Voxel> came_from, Voxel start, Voxel current)
        {
            var path = new HashSet<Voxel>();
